...
export default function ProducerOrdersPage() {
  ...
  const { data: orders = [], isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await apiClient.get(`/api/producer/orders?status=${status === 'all' ? '' : status}`);
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <Tabs defaultValue="all" onValueChange={(value) => setStatus(value as OrderStatus)}>
