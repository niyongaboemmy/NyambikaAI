...
export default function ProducerOrders() {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const response = await apiClient.get("/api/producer/orders");
  ...
  return (
    <div className="pt-10">
      <div className="sticky top-0 z-50 glassmorphism border-b border-white/20 rounded-2xl dark:border-none">
                ...
                <CardContent className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="font-semibold text-primary">
                      {formatPrice(
                        order.items.reduce((a, b) => a + Number(b.price), 0)
                      )}
                    </div>
                    <Button
                      className="gradient-bg text-white"
