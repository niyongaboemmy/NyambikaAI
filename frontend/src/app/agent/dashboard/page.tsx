...
export default function AgentDashboard() {
  ...
  const filteredProducers = producers.filter((producer) => {
    ...
    const matchesFilter =
      ...
      (filterStatus === "expired" &&
        producer.subscription?.status === "expired") ||
  ...
  const stats = {
    ...
    activeSubscriptions: producers.filter(
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-300">
              Manage your producer network and subscriptions
            </p>
          </div>
          ...
          {filteredProducers.map((producer) => (
            ...
            >
                ...
                {producer.subscription ? (
                  <div>
                    ...
                    <p className="text-white font-medium">
                      {producer.subscription.planName}
                    </p>
                    <p className="text-gray-400 text-sm">
                      Expires:{" "}
                      {new Date(
                        producer.subscription.endDate
                      ).toLocaleDateString()}
                    </p>
                  </div>
                ) : (
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="text-center">
                  <p className="text-2xl font-bold text-white">
                    {producer.subscriptionCount}
                  </p>
                  <p className="text-gray-400 text-xs">Subscriptions</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-white">
                    {(producer.totalPaid * 0.4).toLocaleString()}
                  </p>
              ...
              <div className="flex gap-2">
                ...
                !producer.subscription ? (
                  ...
                  <button
                    onClick={() =>
                      router.push(`/agent/producers/${producer.id}`)
                    }
