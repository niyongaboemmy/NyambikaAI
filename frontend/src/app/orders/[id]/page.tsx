...
function OrderDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const confirmOrderReceived = useMutation({
    mutationFn: async () => {
      try {
        const response = await apiClient.post(
          `/api/orders/${resolvedParams.id}/confirm-received`
        );
        return response.data;
      } catch (error) {
        throw new Error(handleApiError(error));
  ...
  const handleConfirmOrder = async () => {
    setConfirming(true);
    try {
      await confirmOrderReceived.mutateAsync();
    } finally {
      setConfirming(false);
    }
  };
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                    ...
                    <span className="hidden sm:inline">
                      {t("orders.detail.aiTrack")}
                    </span>
                    <span className="sm:hidden">
                      {t("orders.detail.aiTrackShort")}
                    </span>
                  </div>
                </div>
              </div>
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-600 dark:text-gray-400">
                        ...
                        {order.items.length === 1
                          ? t("orders.detail.count")
                          : t("orders.detail.countPlural")}
                      </div>
                    </div>
                  </CardTitle>
                </CardHeader>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-gray-600 dark:text-gray-400">
                          {t("orders.detail.itemsLine").replace(
                            "{count}",
                      ...
                      >
                        {confirming ? (
                          ...
                          <>
                            ...
                            <span className="hidden sm:inline">
                              {t("orders.detail.confirmReceived")}
                            </span>
                            <span className="sm:hidden">
                              {t("orders.detail.confirmReceivedShort")}
                            </span>
                          </>
                        )}
                      </Button>
