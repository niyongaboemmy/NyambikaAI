      ...
      <div className="relative z-10 container mx-auto px-4 py-16">
        ...
        <div className="text-center mb-16">
          ...
          <h1 className="text-5xl font-bold text-white mb-6">
            Choose Your
            <span className="bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent"> AI-Powered </span>
            Plan
          </h1>
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Unlock the full potential of your fashion business with our AI-driven subscription plans
          </p>
        </div>
        ...
        {currentSubscription && (
          <div className="mb-12 p-6 bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 rounded-2xl backdrop-blur-xl">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xl font-semibold text-white mb-2">Current Plan: {currentSubscription.plan.name}</h3>
                <p className="text-gray-300">
                  Status: {currentSubscription.subscription.status} • 
                  Expires: {new Date(currentSubscription.subscription.endDate).toLocaleDateString()}
                </p>
              </div>
              <button
          ...
          {plans.map((plan, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`h-full p-8 rounded-3xl backdrop-blur-xl border transition-all duration-500 group-hover:scale-105 ${
                  isPopular
                    ? 'bg-gradient-to-br from-purple-500/20 to-pink-500/20 border-purple-500/50 shadow-2xl shadow-purple-500/25'
                    : 'bg-white/5 border-white/10 hover:border-white/20'
                }`}>
                  {/* Plan Icon */}
                  <div className={`inline-flex p-3 rounded-2xl bg-gradient-to-r ${getPlanGradient(plan.name)} mb-6`}>
                  ...
                  <div className="mb-8">
                    ...
                    {billingCycle === 'annual' && (
                      <p className="text-sm text-green-400 mt-1">
                        Save {Math.round((1 - (parseFloat(plan.annualPrice) / 12) / parseFloat(plan.monthlyPrice)) * 100)}% annually
                      </p>
                    )}
                  </div>
        ...
        <div className="mt-20 text-center">
          ...
          <div className="grid md:grid-cols-4 gap-6 max-w-4xl mx-auto">
            {[
              { icon: '🤖', title: 'AI Try-On Technology', desc: 'Advanced virtual fitting' },
              { icon: '📱', title: 'Mobile Payments', desc: 'MTN & Airtel Money' },
              { icon: '🛡️', title: 'Secure Platform', desc: 'Enterprise-grade security' },
              { icon: '🌍', title: 'Multi-language', desc: 'English, Kinyarwanda, French' },
            ].map((feature, index) => (
