...
export default function SubscriptionManagePage() {
  ...
  const handleUpgradePlan = async (newPlan: SubscriptionPlan, billingCycle: 'monthly' | 'annual') => {
    if (!currentSubscription) return;

    setProcessing(true);
    try {
      const amount = billingCycle === 'monthly' ? newPlan.monthlyPrice : newPlan.annualPrice;
      ...
      if (response.ok) {
        ...
        router.push(`/subscription/payment/${currentSubscription.id}?upgrade=true`);
      } else {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-300 mb-6">You don't have an active subscription. Choose a plan to get started.</p>
          <button
            onClick={() => router.push('/subscription')}
            className="px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl hover:shadow-lg transition-all duration-300"
          >
            Choose a Plan
          </button>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="relative z-10 container mx-auto px-4 py-16">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-white mb-4">Manage Subscription</h1>
          <p className="text-gray-300">Control your subscription settings and billing</p>
        </div>
          ...
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 p-8">
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">{currentSubscription.planName} Plan</h2>
                  <p className="text-gray-300">
                    {currentSubscription.billingCycle === 'monthly' ? 'Monthly' : 'Annual'} billing
                  </p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-3xl font-bold text-white">{parseInt(currentSubscription.amount).toLocaleString()} RWF</p>
                <p className="text-gray-400">per {currentSubscription.billingCycle === 'monthly' ? 'month' : 'year'}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div className="text-center p-4 bg-white/5 rounded-xl">
                ...
                <p className="text-white font-semibold capitalize">{currentSubscription.status}</p>
              ...
              <div className="text-center p-4 bg-white/5 rounded-xl">
                ...
                <p className="text-gray-400 text-sm">
                  {isExpiringSoon() ? `Expires in ${getDaysUntilExpiry()} days` : 'Next billing'}
                </p>
              </div>
            ...
            {isExpiringSoon() && (
              <div className="mb-6 p-4 bg-orange-500/10 border border-orange-500/30 rounded-xl">
                <div className="flex items-start">
                  ...
                  <div>
                    <h4 className="text-orange-300 font-medium mb-1">Subscription Expiring Soon</h4>
                    <p className="text-orange-200 text-sm">
                      Your subscription expires in {getDaysUntilExpiry()} days. 
                      {!currentSubscription.autoRenew && ' Enable auto-renewal or renew manually to avoid service interruption.'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              >
                {currentSubscription.autoRenew ? 'Disable' : 'Enable'} Auto-Renewal
              </button>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-slate-900 rounded-2xl border border-white/20 p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-semibold text-white">Upgrade Your Plan</h3>
                <button
                    ...
                    >
                      ...
                      <div className="mb-6">
                        ...
                        <div className="flex items-baseline">
                          ...
                          <span className="text-green-400 text-xs ml-2">Save 17%</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <button
                          ...
                          disabled={processing}
                          className="w-full py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl hover:shadow-lg transition-all duration-300 text-sm disabled:opacity-50"
                        >
                          Upgrade Annual
                        </button>
                      </div>
