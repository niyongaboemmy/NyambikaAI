...
export default function ProducerDashboard() {
  ...
  const [filterStatus, setFilterStatus] = useState<'all' | 'pending' | 'processing' | 'handled'>('all');
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const mockStats: DashboardStats = {
        ...
        completedOrders: mockOrders.filter(o => o.status === 'handled').length,
        totalRevenue: mockOrders.reduce((sum, o) => sum + parseFloat(o.total), 0),
        ...
        <div className="relative z-10 container mx-auto px-4 py-8">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl hover:shadow-lg transition-all duration-300 flex items-center"
            >
            ...
            <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{stats?.monthlyRevenue.toLocaleString()} RWF</p>
                </div>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-purple-300 font-semibold">{order.total} RWF</p>
                      </div>
                    </div>
                      ...
                      <div className="space-y-2">
                        {order.items.map((item, index) => (
                          <div key={index} className="flex items-center justify-between p-2 bg-white/5 rounded-lg">
                            ...
                            <p className="text-white font-semibold">{item.price} RWF</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {order.sizeEvidenceImages.length > 0 && (
                      <div className="mb-4">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="text-gray-400 text-sm">Size Evidence Photos ({order.sizeEvidenceImages.length})</p>
                        </div>
                        <div className="flex gap-2">
                          {order.sizeEvidenceImages.map((imageUrl, index) => (
                            <div key={index} className="w-16 h-16 bg-white/10 rounded-lg overflow-hidden">
                              <img
                                src={imageUrl}
                    ...
                    {order.notes && (
                      <div className="mb-4">
                        ...
                        <p className="text-gray-300 text-sm bg-white/5 p-3 rounded-lg">{order.notes}</p>
                      </div>
                    )}
                  ...
                  <div className="flex flex-col gap-2 ml-4">
                    <button
                      onClick={() => router.push(`/producer/orders/${order.id}`)}
