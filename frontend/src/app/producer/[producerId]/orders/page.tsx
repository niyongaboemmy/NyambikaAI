...
function ProducerOrdersPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const { data: orders } = await apiClient.get(`/api/orders/producer/${producerId}`);
  ...
  const updateOrderMutation = useMutation({
    mutationFn: async ({ orderId, status, notes }: { orderId: string; status: string; notes: string }) => {
      const { data } = await apiClient.put(`/api/orders/producer/${producerId}`, { orderId, status, notes });
      return data;
    },
    onSuccess: () => {
