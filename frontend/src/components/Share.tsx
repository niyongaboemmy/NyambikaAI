...
import { Share2, Mail, Twitter, Facebook, Linkedin, MessageCircle, Copy, Send, Bookmark, Hash, X } from "lucide-react";
...
export default function Share({
  ...
  const [selected, setSelected] = React.useState<
    "twitter" | "facebook" | "linkedin" | "whatsapp" | "telegram" | "email" | "reddit" | "pinterest" | null
  >(null);
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <div className="absolute inset-0 flex items-end md:items-center justify-center p-0 md:p-4" onClick={() => setOpen(false)}>
              <motion.div
                key="panel"
              ...
              >
                ...
                <div className="flex items-center justify-between px-4 pt-3 md:pt-4">
                  <div className="flex items-center gap-2">
                    {metadata.icon ? (
                      <img src={metadata.icon} alt="icon" className="w-7 h-7 rounded-full object-cover ring-2 ring-white/40" />
                    ) : (
                      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-blue-400 to-purple-500" />
                    )}
                    <div>
                      <div className="text-sm font-semibold text-gray-900 dark:text-white">Share</div>
                      <div className="text-[11px] text-gray-500 dark:text-gray-400 line-clamp-1">{metadata.title}</div>
                    </div>
                  </div>
                  <button
                ...
                <div className="px-4 pb-[calc(env(safe-area-inset-bottom)+12px)] pt-2 md:pb-4">
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-4 gap-2">
                    <PlatformCard selected={selected === "twitter"} label="Twitter / X" onClick={() => setSelected("twitter")} icon={<Twitter className="w-4 h-4" />} color="from-gray-900 to-gray-700" />
                    <PlatformCard selected={selected === "facebook"} label="Facebook" onClick={() => setSelected("facebook")} icon={<Facebook className="w-4 h-4" />} color="from-blue-600 to-blue-400" />
                    <PlatformCard selected={selected === "linkedin"} label="LinkedIn" onClick={() => setSelected("linkedin")} icon={<Linkedin className="w-4 h-4" />} color="from-sky-600 to-sky-400" />
                    <PlatformCard selected={selected === "whatsapp"} label="WhatsApp" onClick={() => setSelected("whatsapp")} icon={<MessageCircle className="w-4 h-4" />} color="from-emerald-600 to-emerald-400" />
                    <PlatformCard selected={selected === "telegram"} label="Telegram" onClick={() => setSelected("telegram")} icon={<Send className="w-4 h-4" />} color="from-sky-500 to-sky-300" />
                    <PlatformCard selected={selected === "email"} label="Email" onClick={() => setSelected("email")} icon={<Mail className="w-4 h-4" />} color="from-rose-500 to-rose-300" />
                    <PlatformCard selected={selected === "reddit"} label="Reddit" onClick={() => setSelected("reddit")} icon={<Hash className="w-4 h-4" />} color="from-orange-500 to-amber-400" />
                    {u.pinterest && (
                      <PlatformCard selected={selected === "pinterest"} label="Pinterest" onClick={() => setSelected("pinterest")} icon={<Bookmark className="w-4 h-4" />} color="from-rose-600 to-pink-500" />
                    )}
                  </div>
                  ...
                  <div className="mt-4 flex flex-col sm:flex-row gap-2">
                    <Button
                      ...
                      onClick={() => {
                        const href =
                          selected === "twitter" ? u.twitter :
                          selected === "facebook" ? u.facebook :
                          selected === "linkedin" ? u.linkedin :
                          selected === "whatsapp" ? u.whatsapp :
                          selected === "telegram" ? u.telegram :
                          selected === "email" ? u.email :
                          selected === "reddit" ? u.reddit :
                          selected === "pinterest" ? u.pinterest :
                          undefined;
                        if (!href) {
                          toast({ title: "Select a platform", description: "Please pick a platform to share.", variant: "default" as any });
                          return;
                        }
...
function PlatformCard({ selected, label, icon, onClick, color }: { selected: boolean; label: string; icon: React.ReactNode; onClick: () => void; color: string }) {
  return (
    <button
      type="button"
    ...
    >
      {/* gradient halo */}
      <span className={`absolute inset-x-6 -top-8 h-16 rounded-full blur-xl opacity-40 group-hover:opacity-70 transition bg-gradient-to-r ${color}`} />
      <span className={`absolute inset-x-10 -bottom-10 h-16 rounded-full blur-xl opacity-30 group-hover:opacity-60 transition bg-gradient-to-r ${color}`} />
      <span className={`relative z-10 flex h-10 w-10 items-center justify-center rounded-lg text-white bg-gradient-to-br ${color} shadow-md shadow-black/10 ${selected ? "scale-110" : ""}`}>
        {icon}
      </span>
      <span className={`relative z-10 text-[12px] mt-1 text-center px-2 line-clamp-1 ${selected ? "font-semibold text-blue-700 dark:text-blue-300" : "text-gray-700 dark:text-gray-300 font-medium"}`}>
        {label}
      </span>
      {selected && (
        <>
          <span className="pointer-events-none absolute inset-0 rounded-xl ring-4 ring-blue-400/80" />
          <span className="absolute top-1.5 right-1.5 z-10 inline-flex items-center justify-center w-5 h-5 rounded-full bg-blue-500 text-white text-[11px] font-bold shadow ring-1 ring-white/80">✓</span>
        </>
      )}
    </button>
  );
