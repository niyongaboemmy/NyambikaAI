    ...
    try {
      ...
      const processingResult = await processImage(
        ...
        if (!processingResult.success) {
          ...
          }, "File uploaded successfully (processing failed)", 201);
        }
      ...
      }, "File uploaded and processed successfully", 201);
    } catch (error) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return sendSuccess(res, {
        files,
      }, `${files.length} files uploaded and processed successfully`, 201);
    } catch (error) {
      console.error("Multiple upload error:", error);
      return sendError(res, 500, "Failed to upload files", error);
    }
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return sendError(res, 500, "Failed to delete file", error);
    }
...
router.get("/uploads/:filename", (req: Request, res: Response) => {
  ...
  fileStream.on("error", (error) => {
    ...
    if (!res.headersSent) {
      return sendError(res, 500, "Error streaming file", error);
    }
